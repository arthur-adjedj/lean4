...
private def checkClass (rs : Array ElabHeaderResult) : TermElabM Unit := do
  if rs.size > 1 then
    for r in rs do
...
private partial def checkHeaders (rs : Array ElabHeaderResult) (numParams : Nat) (i : Nat) (firstType? : Option Expr) : TermElabM Unit := do
  trace[Elab.inductive] "checkHeaders {i}"
  if h : i < rs.size then
    let type ← checkHeader rs[i] numParams firstType?
    let r := rs[i]
    checkHeaders rs numParams (i+1) type
where
  checkHeader (r : ElabHeaderResult) (numParams : Nat) (firstType? : Option Expr) : TermElabM Expr := do
    let type := r.type
...
private def elabHeadersAux (views : Array InductiveView) (i : Nat) (acc : Array ElabHeaderResult) (k : Array ElabHeaderResult → TermElabM α): TermElabM α :=
  Term.withAutoBoundImplicitForbiddenPred (fun n => views.any (·.shortDeclName == n)) do
    if h : i < views.size then
      let view := views[i]
      let header ← withRef view.ref <| Term.withAutoBoundImplicit <| Term.elabBinders view.binders.getArgs fun params => do
        ...
        return { levelNames, numParams := params.size, type, view, origParams := params : PreElabHeaderResult }
      withAuxDecl view.shortDeclName header.type header.view.declName fun indFVar =>
        elabHeadersAux views (i+1) (acc.push {header with indFVar }) k
    else
      k acc
...
-/
private def elabHeaders (views : Array InductiveView) (k : Array ElabHeaderResult → TermElabM α) : TermElabM α :=
  elabHeadersAux views 0 #[] fun rs => do
    if rs.size > 1 then
      checkUnsafe rs
      checkClass rs
  ...
  forallBoundedTelescope r0.type r0.numParams fun params _ => withRef r0.view.ref do
    let rec loop (i : Nat) (indFVars : Array Expr) := do
      if h : i < rs.size then
        ...
        loop (i+1) (indFVars.push r.indFVar)
      else
        x params indFVars
    loop 0 #[]
where
  transformHeader (declName : Name) (indFVar : Expr) (headerType : Expr) : MetaM Expr :=
    let visit (e : Expr) : MetaM TransformStep := do
      let f := e.getAppFn
      if f.constName? == declName then
        let args := e.getAppArgs
        return TransformStep.done (mkAppN f args)
      else
        return .continue
    transform headerType (pre := visit)
...
private def replaceIndFVarsWithConsts (views : Array InductiveView) (indFVars : Array Expr) (ctorFVars : Array (Array Expr)) (levelNames : List Name)
  ...
  let indTypes ← indTypes.mapM fun indType => do
    ...
    let ctors ← indType.ctors.mapM fun ctor => do
      return { ctor with type := ← replaceinType ctor.type}
    return { indType with type, ctors }
  trace[Elab.inductive] "final inductives :\n {repr indTypes}"
  return indTypes
...
private def mkInductiveDecl (vars : Array Expr) (elabs : Array InductiveElabStep1) : TermElabM FinalizeContext :=
  ...
  withRef view0.ref <| Term.withLevelNames allUserLevelNames <| elabHeaders views fun rs => do
    Term.synthesizeSyntheticMVarsNoPostponing
    ...
    let res ← withInductiveLocalDecls rs fun params indFVars => do
      ...
      withElabCtors params rs indFVars views elabs fun elabs' indTypesArray ctorFVars => do
        ...
        let indTypesWithFVars := indTypesArray.toList
        let u ← getResultingUniverse indTypesWithFVars
        let univToInfer? ← shouldInferResultUniverse u
        withUsed elabs' vars indTypesWithFVars fun vars => do
          let numVars   := vars.size
          let numParams := numVars + numExplicitParams
          let indTypesWithFVars ← updateParams vars indTypesWithFVars
          let indTypesWithFVars ← withoutExporting do
            if let some univToInfer := univToInfer? then
              updateResultingUniverse views numParams (← levelMVarToParam indTypesWithFVars univToInfer)
            else
              propagateUniversesToConstructors numParams indTypesWithFVars
              levelMVarToParam indTypesWithFVars none
          withoutExporting do
            checkResultingUniverses views elabs' numParams indTypesWithFVars
          elabs'.forM fun elab' => elab'.finalizeTermElab
          let usedLevelNames := collectLevelParamsInInductive indTypesWithFVars
          match sortDeclLevelParams scopeLevelNames allUserLevelNames usedLevelNames with
          | .error msg      => throwErrorAt view0.declId msg
          | .ok levelParams => do
            let indTypes ← replaceIndFVarsWithConsts views indFVars ctorFVars levelParams numVars numParams indTypesWithFVars
            let mut hasIndInds := false
            for i in [:indTypes.length] do
              if indTypes[i]!.type != indTypesWithFVars[i]!.type then
                trace[Elab.Inductive] m!"Is Inductive-inductive ({indentExpr indTypes[i]!.type} != {indentExpr indTypesWithFVars[i]!.type}"
                hasIndInds := true
            ...
            pure {
              elabs := elabs', levelParams, params := vars ++ params, replaceIndFVars, hasIndInds,
...
private def mkAuxConstructions (declNames : Array Name) (hasIndInds : Bool) : TermElabM Unit := do
  let env ← getEnv
  ...
  for n in declNames do
    mkRecOn n
    if !hasIndInds && hasUnit then mkCasesOn n
    if !hasIndInds && hasNat then mkCtorIdx n
    if !hasIndInds && hasNat then mkCtorElim n
    if !hasIndInds && hasUnit && hasEq && hasHEq then mkNoConfusion n
    if !hasIndInds && hasUnit && hasProd then mkBelow n
  if !hasIndInds && hasUnit && hasProd then
    for n in declNames do mkBRecOn n
...
private def elabInductiveViews (vars : Array Expr) (elabs : Array InductiveElabStep1) : TermElabM FinalizeContext := do
  ...
  withExporting (isExporting := !isPrivateName view0.declName) do
    ...
    withoutExporting (when := view0.ctors.any (isPrivateName ·.declName)) do
      mkAuxConstructions (elabs.map (·.view.declName)) res.hasIndInds
      unless view0.isClass || res.hasIndInds do
